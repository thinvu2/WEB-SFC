...
namespace SN_API.Controllers.Config
{
    ...
    public class ConfigJIGController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> GetConfigJIGContent(ConfigJigElement model)
        {
            ...
            {
                strGetData = $@" select A.TYPE_VALUE ,A.ATTRIBUTE_VALUE,A.ATTRIBUTE_DESC1,A.EMP_NO,A.INPUT_TIME,ROWIDTOCHAR(ROWID) ID from SFIS1.C_MODEL_ATTR_CONFIG_T A 
                WHERE ATTRIBUTE_NAME = 'CHECK_JIG' AND (UPPER(A.TYPE_VALUE) = '{model.TYPE_VALUE.ToUpper()}' or ATTRIBUTE_DESC1 = '{model.TYPE_VALUE.ToUpper()}') ";
            }
