...
namespace SN_API.Controllers
{
    ...
    public class Config76Controller : ApiController
    {
        ...
        {
            string strGetData = "select mo_number from sfism4.r105 where 1 = 1 and MO_CREATE_DATE >= to_date('2024-01-01','YYYY-DD-MM') order by mo_create_date desc";
            DataTable dtCheck = DBConnect.GetData(strGetData, model.database_name);
            return Request.CreateResponse(HttpStatusCode.OK, new { result = "ok", data = dtCheck });
        }
        /// <summary>
        ...
        public async Task<HttpResponseMessage> DeleteConfigTA_(Config76Element model)
        {
            ...
            string strDelete = $" delete  from sfis1.C_MODEL_CONFIRM_T where rowid='{model.ID}' ";
            try
            {
