...
namespace SN_API.Controllers.QualcommApps
{
    ...
    {
        [System.Web.Http.Route("QReceiptDataTable")]
        [System.Web.Http.HttpGet]
        public async Task<HttpResponseMessage> QReceiptDataTable(string databaseName, string inFunc, string inData, string empNo, string dateFrom, string dateTo)
        {
            try
            {
                string checkExistsProcedure = "SFIS1.SP_QRECEIPT";
                string procedureResult = "";
                List<Dictionary<string, object>> cursorData1 = new List<Dictionary<string, object>>();
                List<Dictionary<string, object>> cursorData2 = new List<Dictionary<string, object>>();
                ...
                using (var connection = new OracleConnection(connectionString))
                {
                    using (var command = new OracleCommand(checkExistsProcedure, connection))
                    {
                        ...
                        command.Parameters.Add("OUT_DT1", OracleDbType.RefCursor).Direction = ParameterDirection.Output;
                        command.Parameters.Add("OUT_DT2", OracleDbType.RefCursor).Direction = ParameterDirection.Output;
                        ...
                        procedureResult = command.Parameters["RES"].Value.ToString();
                            try
                            {
                                var reader = ((OracleRefCursor)command.Parameters["OUT_DT1"].Value).GetDataReader();
                            ...
                            {
                                cursorData1 = new List<Dictionary<string, object>>();
                            }

                        try
                        {
                            var reader = ((OracleRefCursor)command.Parameters["OUT_DT2"].Value).GetDataReader();
                        ...
                        {
                            cursorData2 = new List<Dictionary<string, object>>();
                        }
                ...
                {
                    return Request.CreateResponse(HttpStatusCode.OK, new { result = "ok", data = cursorData1, dataTable = cursorData2});
                }
                else
                {
