    ...
    <div id="grid-container">
      ...
      <template v-else>
        ...
        <datalist id="datalistOptionsCopyNotDf">
          <option
            v-for="(option, index) in limitedOptionsCopyNotDf"
            :key="index"
          >
        ...
        <template v-if="valueInput === false">
          ...
          <datalist id="datalistOptionsCopyDf">
            <option
              v-for="(option, index) in limitedOptionsCopyDf"
              :key="index"
            >
              {{ option.EMP_COPYDF }}
            </option>
      ...
      <div class="not-defineApp" v-if="showFormTableAndSave">
        ...
        <select
          v-model="selectedNotDf"
          @change="selectNotDfApp"
          class="custom-select"
        >
          <option
            v-for="(item, index) in selectListNotDefineApp"
            v-bind:key="index"
            class="custom-option"
          >
            {{ item.SPRG_NAME }}
      ...
      <div class="defineApp" v-if="showFormTableAndSave">
        ...
        >
          <option
            v-for="(item, index) in selectListDefineApp"
            v-bind:key="index"
            class="custom-option"
          >
            {{ item.DPRG_NAME }}
        ...
        <template v-if="ListNotDefineAppHeader">
          <table>
            <thead>
              <tr>
                <template
                  v-for="(item, index) in ListNotDefineAppHeader"
                  :key="index"
                >
                  <th
                    v-if="
                      item == 'PRG_NAME' || item == 'FUN' || item == 'PASSW'
                    "
                  >
                    {{ item }}
                  </th>
            ...
            <tbody>
              <tr
                v-for="(item, index) in ListNotDefineApp"
                :key="index"
      ...
      <div class="moveAndSaveData" v-if="showFormTableAndSave">
        <button
          class="moveNotDefineButton"
          @click="moveSelectedItemsToDefinedApp"
        >
          <svg
        ...
        <button
          class="moveDefineButton"
          @click="moveSelectedItemsToNotDefinedApp"
        >
          <svg
        ...
        <template v-if="valueInput === false">
          <table>
            <thead>
              <tr>
                <template
                  v-for="(item, index) in ListDefineAppHeader"
                  :key="index"
                >
                  <th
                    v-if="
            ...
            <tbody>
              <tr
                v-for="(item, index) in ListDefineApp"
                :key="index"
                @click="toggleSelect(item)"
                :class="{ selected: item.isSelected }"
              >
...
export default {
  ...
  methods: {
    async GetEmp() {
      ...
      try {
        let { data } = await Repository.getApiServer(
          `getEmpNo?database_name=${databaseName}`
        );
    ...
    async GetEmpCopyDf() {
      ...
      try {
        let { data } = await Repository.getApiServer(
          `getEmpCopyDf?database_name=${databaseName}`
        );
    ...
    async GetEmpCopyNotDf() {
      ...
      try {
        let { data } = await Repository.getApiServer(
          `getEmpCopyNotDf?database_name=${databaseName}`
        );
    ...
    async GetEmpList() {
      ...
      try {
        let response = await Repository.getApiServer(
          `selectMultipleEmp?database_name=${databaseName}&LISTINPUTEMP=${encodeLISTINPUTEMP}`
        );
    ...
    async filterEmp() {
      ...
      this.filteredOptions = this.ListEmpAll.filter(function(option) {
        if (option && option.EMP_NO_NAME && self.model.EMP_NO_NAME) {
          return option.EMP_NO_NAME.toLowerCase().includes(
            self.model.EMP_NO_NAME.toLowerCase()
          );
        }
        return false;
      });

      const allIndex = this.filteredOptions.findIndex(
        (option) => option.EMP_NO_NAME === "ALL"
      );
      if (allIndex > -1) {
    ...
    async filterListEmpCopyDf() {
      ...
      this.filteredOptionsCopyDf = this.ListEmpCopyDf.filter(function(option) {
        if (option && option.EMP_COPYDF && self.model.EMP_COPYDF) {
          return option.EMP_COPYDF.toLowerCase().includes(
            self.model.EMP_COPYDF.toLowerCase()
          );
        }
        return false;
      });
    ...
    async filterListEmpCopyNotDf() {
      const self = this;
      this.filteredOptionsCopyNotDf = this.ListEmpCopyNotDf.filter(function(
        option
      ) {
        if (option && option.EMP_COPYNOTDF && self.model.EMP_COPYNOTDF) {
          return option.EMP_COPYNOTDF.toLowerCase().includes(
            self.model.EMP_COPYNOTDF.toLowerCase()
          );
        }
        return false;
      });
    ...
    async selectNotDefineApp() {
      ...
      try {
        let { data } = await Repository.getApiServer(
          `selectGetNotDefineApp?database_name=${databaseName}&EMP_COPYNOTDF=${EMP_COPYNOTDF}&value=${value}`
        );
    ...
    async selectDefineApp() {
      ...
      try {
        let { data } = await Repository.getApiServer(
          `selectGetDefineApp?database_name=${databaseName}&EMP_NO_NAME=${EMP_NO_NAME}&EMP_COPYDF=${EMP_COPYDF}&value=${value}&valueInput=${valueInput}&LISTINPUTEMP=${encodeLISTINPUTEMP}`
        );
    ...
    async notDefineApp() {
      ...
      try {
        let { data } = await Repository.getApiServer(
          `getNotDefineApp?database_name=${databaseName}&EMP_NO_NAME=${EMP_NO_NAME}&SPRG_NAME=${SPRG_NAME}&EMP_COPYNOTDF=${EMP_COPYNOTDF}&value=${value}`
        );
    ...
    async defineApp() {
      ...
      try {
        let { data } = await Repository.getApiServer(
          `getDefineApp?database_name=${databaseName}&EMP_NO_NAME=${EMP_NO_NAME}&DPRG_NAME=${DPRG_NAME}&PRIVILEGE=${PRIVILEGE}&EMP_COPYDF=${EMP_COPYDF}&value=${value}&valueInput=${valueInput}&LISTINPUTEMP=${encodeLISTINPUTEMP}`
        );
    ...
    moveSelectedItemsToDefinedApp() {
      const selectedItems = this.ListNotDefineApp.filter(
        (item) => item.isSelected
      );
      const initiallyInNotListDefineApp = selectedItems.filter(
        (item) => this.TEST.includes(item) || !this.TEST1.includes(item)
      );
      ...
      this.ListNotDefineApp = this.ListNotDefineApp.filter(
        (item) => !item.isSelected
      );
      selectedItems.forEach((item) => {
    ...
    moveSelectedItemsToNotDefinedApp() {
      const selectedItems = this.ListDefineApp.filter(
        (item) => item.isSelected
      );
      const initiallyInListDefineApp = selectedItems.filter(
        (item) => this.TEST1.includes(item) || !this.TEST.includes(item)
      );

      this.ListDefineApp = this.ListDefineApp.filter(
        (item) => !item.isSelected
      );
      selectedItems.forEach((item) => {
        item.isSelected = false;
