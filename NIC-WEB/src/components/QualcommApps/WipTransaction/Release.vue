    ...
    <div class="input-center">
      ...
      <div class="optionInput row col-12">
        ...
        <div class="valueReceviceLot col-3">
          <input class="form-control" v-model="reasonValue" />
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      IN_STAGE_IN: "",
      action: "Release",
      transaction: "WIPReleaseTransaction",
      filteredList: [],
      ListTotal: [],
      showDropdown: false,
    };
  ...
  methods: {
    ...
    async loadCreateLot() {
      ...
      var { data } = await Repository.getApiServer(
        `GetLoadLotNoQWip?database_name=${databaseName}&type=RELEASE_GET_LOTNO`
      );
      if (data.result == "ok") {
    ...
    async CheckQty_Qwip(qty) {
      ...
      if (data.result == "ok") {
        this.ListTotal = data.data;
        var total = this.ListTotal[0].TOTAL;
        if (parseInt(total) < 0) {
          this.Qty = 0;
